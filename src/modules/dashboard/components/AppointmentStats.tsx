...
export default function AppointmentStats({ startDate, endDate }: AppoimentStatsProps) {
    ...
    useEffect(() => {
        if (data) {
            const newDailyData = data.dailyStats.map((item: any) => ({
                day: item.day,
                count: item.count,
                percent: `${((item.count / data.totalAppointments) * 100).toFixed(2)}%`,
            }));

            const newTimeSlotData = data.timeSlotStats
                .map((item: any) => ({
                    time: item.timeSlot,
                    count: item.appointments,
                    percent: `${((item.appointments / data.totalAppointments) * 100).toFixed(2)}%`,
                }))
                // Sắp xếp các khung giờ theo số lượng cuộc hẹn giảm dần
                .sort((a: any, b: any) => b.count - a.count)
                // Chỉ lấy 7 khung giờ đầu tiên
                .slice(0, 7);

            setDailyData(newDailyData);
            setTimeSlotData(newTimeSlotData);
        }
    }, [data]); // Đảm bảo hook useEffect không thay đổi thứ tự
