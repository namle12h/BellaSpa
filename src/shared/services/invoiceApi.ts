import { useMutation, useQuery, useQueryClient } from "@tanstack/react-query";
import { axiosClient } from "../lib/axiosClient";
import { message } from "antd";
import type {
  ApiInvoice,

} from "../../shared/types/type";

export const createInvoice = async (data: any) => {
  // 👉 Gửi JSON thuần, không cần FormData
  const res = await axiosClient.post("/invoices", data, {
    headers: { "Content-Type": "application/json" },
  });
  return res.data;
};
export interface PaginatedInvoice {
  content: ApiInvoice[];
  totalPages: number;
  totalElements: number;
  size: number;
  number: number;
}

// export const getAllInvoices = async (): Promise<ApiInvoice[]> => {
//   const res = await axiosClient.get<ApiInvoice[]>("/invoices", {
//     headers: { "Content-Type": "application/json" },
//   });
//   return res.data;
// };
export const getAllInvoices = async (
  page = 0,
  size = 10,
  sortBy = "createdAt"
): Promise<PaginatedInvoice> => {
  const res = await axiosClient.get<PaginatedInvoice>(`/invoices`, {
    params: { page, size, sortBy },
    headers: { "Content-Type": "application/json" },
  });
  return res.data;
};
/**
 * ⚙️ Hook React Query để tạo hóa đơn
 */
export const useCreateInvoice = () => {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: createInvoice,

    onSuccess: (res) => {
      console.log("📦 Hóa đơn đã tạo:", res);

      // Làm mới danh sách hóa đơn (nếu có)
      queryClient.invalidateQueries({ queryKey: ["invoices"] });
    },

    onError: (error: any) => {
      console.error("❌ Lỗi khi tạo hóa đơn:", error.response || error);
      message.error(
        error?.response?.data?.message || "❌ Thanh toán thất bại! Vui lòng thử lại."
      );
    },
  });
};

export const useGetAllInvoice = (page: number, size = 10, sortBy = "createdAt") => {
  return useQuery({
   queryKey: ["invoices", page, size, sortBy], // ✅ thêm vào queryKey để cache theo từng trang
    queryFn: () => getAllInvoices(page, size, sortBy),
  });
};
