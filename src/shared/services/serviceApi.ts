...
export const fetchAllServices = async () => {
    // Giả định API endpoint trả về tất cả. Nếu không, dùng limit lớn
    const response = await axiosClient.get(`/services`, {
        params: { page: 1, limit: 9999 },
    });
    // Nếu API trả về Data object, lấy mảng data bên trong
...
export const useAllServices = () => {
    return useQuery({
        queryKey: ["all-services-lookup"],
        queryFn: fetchAllServices,
        staleTime: 1000 * 60 * 60, // Cache lâu hơn vì đây là dữ liệu tĩnh
    });
};

// 🟡 Lấy TẤT CẢ sản phẩm (không phân trang)
// Dựa trên useProducts đã có, nhưng dùng cho lookup
export const fetchAllProducts = async () => {
    const res = await axiosClient.get(`/products?page=1&limit=9999`);
